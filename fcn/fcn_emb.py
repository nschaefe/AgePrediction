...
def main(hiddens, run_dir):
    ...
    x_train = torch.tensor(df_train.values[:, 21:-2].astype(np.float32)).cuda()
    ...
    x_test = torch.tensor(df_test.values[:, 21:-2].astype(np.float32)).cuda()
...
def optimize_layers(indim=128):
    ...
    for layer_count in num_layers:
        hiddens = [256]  # first layer always has 256
